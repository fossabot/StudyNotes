...
<body>
   ...
   <script type="text/javascript">
      ...
      $(":button:last").click(function () {
         //请求URL
         var url = "${pageContext.request.contextPath}/emp/maptojson.action";
         var sendData = null;
         $.post(url,sendData,function (data,textStatus,jqXHR) {
...
概述：Spring MVC的==拦截器==（Interceptor）与Java Servlet==过滤器==（Filter）类似，它主要用于拦截用户的请求并做响应的处理。通常应用在权限验证，记录请求信息的日志、判断用户是否登录等功能上
...
概述：让自定义拦截器生效需要在Spring MVC配置文件中进行配置
...
http://localhost:8080/student/list.action=>非法攻击
```
...
在Spring MVC应用的开发中，不管是对底层数据库操作，还是业务层、控制层操作，都不可避免的遇到各种预知的、不可预知的异常需要处理。如果每个过程都单独处理异常，那么系统的代码耦合度高，工作量大且不好统一，以后维护的工作量也很大，如果能将所有类型的异常处理从各层解耦出来，这样既保证了相关处理功能单一，又实现了异常信息的统一处理和维护
...
<beans xmlns="http://www.springframework.org/schema/beans"
    ...
    <!-- 开启注解扫描 -->
    <context:component-scan base-package="app14"/>
    ...
    <!-- 全局异常处理 -->
    <bean class="app14.CustomExceptionResolver"/>
</beans>
```
